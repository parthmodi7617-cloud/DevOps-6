...
- Analyzes effect of activation functions and hidden layers
"""
...
# ── Experiment A: Different Activation Functions ───────────────────────────────
print("=" * 60)
...
# ── Experiment B: Different Number of Hidden Layers ───────────────────────────
print("\n" + "=" * 60)
...
# Plot A1: Accuracy curves per activation
ax = axes[0, 0]
...
# Plot A2: Loss curves per activation
ax = axes[0, 1]
for act, hist in act_histories.items():
...
# Plot B1: Accuracy curves per layer config
ax = axes[1, 0]
...
# Plot B2: Bar chart – final test accuracy
ax = axes[1, 1]
names = list(layer_results.keys())
accs  = [layer_results[n][1] for n in names]
bars  = ax.bar(names, accs, color=['steelblue', 'orange', 'green'])
ax.set_title("Hidden Layers – Final Test Accuracy")
ax.set_ylabel("Accuracy"); ax.set_ylim(0.9, 1.0)
